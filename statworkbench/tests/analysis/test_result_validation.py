...
R (v4.6.0) 계산 결과를 기준값으로 삼아 StatWorkbench 분석 결과의
...
import statworkbench.analysis.descriptive as mod_descriptive
import statworkbench.analysis.ttests as mod_ttests
import statworkbench.analysis.correlation as mod_correlation
import statworkbench.analysis.regression as mod_regression
import statworkbench.analysis.normality as mod_normality
import statworkbench.analysis.anova as mod_anova
import statworkbench.analysis.frequencies as mod_frequencies
...
class TestAnovaValidation:
    ...
    def test_statworkbench_anova_produces_f(self):
        rng = np.random.default_rng(1)
        df = pd.DataFrame({
