"""Custom exception hierarchy for the StatWorkbench package.

All exceptions raised by StatWorkbench inherit from the base
:class:`StatWorkbenchError` to enable unified error handling across
the application.
"""


class StatWorkbenchError(Exception):
    """Base exception for all StatWorkbench errors.
...
class VariableError(StatWorkbenchError):
...
class DatasetError(StatWorkbenchError):
...
class AnalysisError(StatWorkbenchError):
...
class ImportError(StatWorkbenchError):
...
class ValidationError(StatWorkbenchError):
...
class ProjectError(StatWorkbenchError):
...
class IORError(StatWorkbenchError):
