...
class SettingsManager:
    ...
    def __init__(self) -> None:
        self._settings = QSettings("NuriStat", "NuriStat")
    ...
    def load_window_size(self) -> QSize:
        """저장된 윈도우 크기 반환."""
        val = self._settings.value("window/size", QSize(1400, 900))
        return val if isinstance(val, QSize) else QSize(1400, 900)

    def load_window_position(self) -> QPoint:
        """저장된 윈도우 위치 반환."""
        val = self._settings.value("window/position", QPoint(100, 100))
        return val if isinstance(val, QPoint) else QPoint(100, 100)

    def load_window_maximized(self) -> bool:
        """저장된 최대화 상태 반환."""
        val = self._settings.value("window/maximized", False)
