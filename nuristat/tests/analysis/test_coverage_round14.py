...
# ─────────────────────────────────────────────────────────────────────────────
# app.py — NuriStatApp (QApplication + MainWindow 완전 모킹)
# ─────────────────────────────────────────────────────────────────────────────

class TestNuriStatApp:
    def test_run_and_get_window(self):
        """app.py L8-39: NuriStatApp.__init__, run(), get_window()."""
        mock_qapp = MagicMock()
        mock_qapp.return_value.exec.return_value = 0
        mock_window = MagicMock()

        # sys.modules 조작 없이 모듈 속성만 직접 패치 (numpy C 확장 재로딩 방지)
        with patch("nuristat.app.QApplication", mock_qapp), \
             patch("nuristat.app.MainWindow", mock_window):
...
class TestMain:
    def test_main_success(self):
        ...
        with patch("nuristat.app.NuriStatApp", mock_app_cls):
            from nuristat.main import main
            result = main()
            assert result == 0
...
class TestResultTableHtmlException:
    def test_image_render_exception_returns_fallback(self):
        ...
        with patch("nuristat.analysis.result.base64.b64encode",
...
class TestRegressionStepwiseException:
    ...
    def test_forward_ols_exception_continue(self):
        ...
        with patch("nuristat.analysis.regression.sm.OLS", patched_ols):
            result = run_analysis(ds, {
...
class TestTwoWayAnovaUncovered:
    ...
    def test_anova_table_missing_key_continue(self):
        ...
        with patch("nuristat.analysis.two_way_anova.sm.stats.anova_lm", patched_anova):
            result = run_analysis(ds, {
                "variables": {"dependent": "y", "factor_a": "fa", "factor_b": "fb"},
    ...
    def test_post_hoc_exception_adds_warning(self):
        ...
        ds = self._ds()
        with patch("nuristat.analysis.two_way_anova._run_post_hoc",
...
class TestSurvivalAnalysisException:
    ...
    def test_prepare_frame_exception(self):
        ...
        ds = self._ds()
        with patch("nuristat.analysis.survival_analysis.prepare_analysis_frame",
...
class TestDiscriminantAnalysisUncovered:
    ...
    def test_prepare_frame_exception(self):
        ...
        ds = self._ds()
        with patch("nuristat.analysis.discriminant_analysis.prepare_analysis_frame",
                   side_effect=RuntimeError("frame fail")):
            result = run_analysis(ds, {
...
class TestFactorAnalysisException:
    ...
    def test_prepare_frame_exception(self):
        ...
        ds = self._ds()
        with patch("nuristat.analysis.factor_analysis.prepare_analysis_frame",
                   side_effect=RuntimeError("frame fail")):
            result = run_analysis(ds, {
...
class TestCohensKappaException:
    ...
    def test_compute_kappa_exception(self):
        ...
        ds = self._ds()
        with patch("nuristat.analysis.cohens_kappa._compute_kappa",
...
class TestClusterAnalysisException:
    ...
    def test_prepare_frame_exception(self):
        ...
        ds = self._ds()
        with patch("nuristat.analysis.cluster_analysis.prepare_analysis_frame",
                   side_effect=RuntimeError("frame fail")):
            result = run_analysis(ds, {
...
class TestRepeatedMeasuresUncovered:
    ...
    def test_sphericity_violation_warning(self):
        ...
        from nuristat.analysis import repeated_measures_anova as rma_mod
...
class TestRegistryUncovered:
    def test_registers_planned_analysis(self):
        ...
        import nuristat.analysis.registry as reg_mod
...
class TestNonparametricException:
    ...
    def test_test_function_exception(self):
        ...
        ds = self._ds()
        with patch("nuristat.analysis.nonparametric._mann_whitney",
...
class TestLogisticRegressionRocException:
    ...
    def test_roc_exception_logged(self):
        ...
        with patch("nuristat.analysis.logistic_regression._SKLEARN_AVAILABLE", False), \
             patch("nuristat.analysis.logistic_regression._trapz",
...
class TestAncovaUncovered:
    ...
    def test_anova_table_missing_key_continue(self):
        ...
        with patch("nuristat.analysis.ancova.sm.stats.anova_lm", patched_anova):
            result = run_analysis(ds, {
                "variables": {
...
class TestTextMiningFontNotFound:
    def test_find_font_returns_none(self):
        ...
        from nuristat.analysis import text_mining as tm_mod
        import os
...
class TestManovaEffectKeyNone:
    def test_effect_key_none_returns_empty_rows(self):
        ...
        from nuristat.analysis.manova import _multivariate_tests
        ...
        with patch("nuristat.analysis.manova.MANOVA") as mock_manova_cls:
