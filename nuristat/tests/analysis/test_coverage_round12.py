...
class TestManovaHelpers:
    ...
    def test_post_hoc_group_less_than_2_continue(self):
        ...
        from nuristat.analysis.manova import _post_hoc
        df = pd.DataFrame({
            "group": ["A"] * 20 + ["B"] * 20 + ["C"] * 1,
    ...
    def test_multivariate_tests_manova_exception(self):
        ...
        df = self._base_df()
        with patch("nuristat.analysis.manova.MANOVA") as mock_manova:
    ...
    def test_univariate_tests_empty_factor_row_continue(self):
        ...
        from nuristat.analysis.manova import _univariate_tests
        ...
        df = self._base_df()
        with patch("nuristat.analysis.manova.sm") as mock_sm:
            mock_sm.stats.anova_lm.return_value = mock_at
            with patch("nuristat.analysis.manova.ols") as mock_ols:
    ...
    def test_univariate_tests_ols_exception(self):
        ...
        from nuristat.analysis.manova import _univariate_tests
        df = self._base_df()
        with patch("nuristat.analysis.manova.ols", side_effect=RuntimeError("ols fail")):
            rows = _univariate_tests(df, ["y1", "y2"], "group", True)
        assert isinstance(rows, list)
...
class TestExploreExceptions:
    ...
    def test_cps_exception(self):
        ...
        from nuristat.analysis.explore import run_analysis
        ds = self._ds()
        with patch("nuristat.analysis.explore._build_case_processing_summary",
    ...
    def test_analysis_loop_exception(self):
        ...
        from nuristat.analysis.explore import run_analysis
        ds = self._ds()
        with patch("nuristat.analysis.explore._compute_explore_stats",
...
class TestSensitivitySpecificityExceptions:
    ...
    def test_compute_2x2_exception(self):
        ...
        ds = self._ds()
        with patch("nuristat.analysis.sensitivity_specificity._compute_2x2",
    ...
    def test_diagnostic_metrics_exception(self):
        ...
        from nuristat.analysis.sensitivity_specificity import run_analysis
        ds = self._ds()
        with patch("nuristat.analysis.sensitivity_specificity._diagnostic_metrics",
