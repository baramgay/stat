...
class TestMissingPolicyTopLevelKey:
    ...
    def test_pca_missing_policy_fallback(self):
        from nuristat.analysis.pca import run_analysis
        ds = self._pca_ds()
        spec = {
            "missing_policy": "invalid_xyz",
...
class TestPcaExceptionPaths:
    ...
    def test_kmo_linalg_error(self):
        ...
        from nuristat.analysis.pca import _kmo_bartlett
        rng = np.random.default_rng(1)
    ...
    def test_rotation_lstsq_exception_returns_loadings(self):
        ...
        from nuristat.analysis.pca import _rotate
        rng = np.random.default_rng(3)
    ...
    def test_scree_plot_exception_returns_none(self):
        ...
        from nuristat.analysis.pca import _scree_plot
...
class TestTwoWayAnovaExceptions:
    ...
    def test_profile_plot_exception_returns_none(self):
        ...
        from nuristat.analysis.two_way_anova import _profile_plot_two_way
        rng = np.random.default_rng(1)
        data = pd.DataFrame({
...
class TestMixedAnovaExceptions:
    ...
    def test_mauchly_exception_returns_default_eps(self):
        ...
        from nuristat.analysis.mixed_anova import _mauchly_test
    ...
    def test_bonferroni_between_ms_nan_continue(self):
        ...
        from nuristat.analysis.mixed_anova import _bonferroni_between
        from nuristat.analysis.result import AnalysisResult
        rng = np.random.default_rng(2)
        result_obj = AnalysisResult(id="test", title="test")
        data = pd.DataFrame({
    ...
    def test_profile_plot_exception_returns_none(self):
        ...
        from nuristat.analysis.mixed_anova import _profile_plot_mixed
        rng = np.random.default_rng(3)
        data = pd.DataFrame({
            "group": ["A"] * 10 + ["B"] * 10,
...
class TestAncovaExceptions:
    ...
    def test_homogeneity_exception_adds_warning(self):
        ...
        with patch("nuristat.analysis.ancova.ols", side_effect=ols_side_effect):
            result = run_analysis(ds, {
                "variables": {"dependent": "y", "factor": "group", "covariates": ["cov"]},
    ...
    def test_ancova_model_error_adds_warning(self):
        ...
        from nuristat.analysis.ancova import run_analysis
        ds = self._ds()
        with patch("nuristat.analysis.ancova.ols", side_effect=RuntimeError("model fail")):
            result = run_analysis(ds, {
                "variables": {"dependent": "y", "factor": "group", "covariates": ["cov"]},
                "options": {},
            })
