...
from nuristat.analysis.factor_analysis import run_analysis, _add_kmo_bartlett
...
class TestSklearnNotAvailable:
    ...
    def test_sklearn_unavailable_warns(self, fa_dataset):
        spec = {"variables": {"variables": ["v1", "v2", "v3", "v4", "v5"]}}
        with patch("nuristat.analysis.factor_analysis._SKLEARN_AVAILABLE", False):
            result = run_analysis(fa_dataset, spec)
...
class TestKMOInterpretation:
    ...
    def test_kmo_marvelous(self):
        ...
        X = self._make_X(1)
        with patch(
            "nuristat.analysis.factor_analysis._add_kmo_bartlett",
        ):
            pass  # 직접 호출로 대체
    ...
    def test_kmo_mediocre_branch(self):
        ...
        import nuristat.analysis.factor_analysis as fa_mod
...
class TestKMOBartlettException:
    ...
    def test_kmo_bartlett_full_exception(self, fa_dataset):
        ...
        with patch("nuristat.analysis.factor_analysis.np.corrcoef",
...
class TestEFAFallbackToPCA:
    ...
    def test_efa_fail_falls_back_to_pca(self, fa_dataset):
        ...
        with patch(
            "nuristat.analysis.factor_analysis.FactorAnalysis",
        ) as mock_fa_cls:
            mock_fa = MagicMock()
