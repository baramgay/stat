...
# nuristat 전체 서브모듈을 자동 수집 — 레지스트리가 분석 모듈을 동적 import하므로
# 수동 나열 대신 collect_submodules로 누락 없이 번들한다.
_sw_modules = collect_submodules('nuristat')
_ext_modules = [
...
a = Analysis(
    ...
    binaries=[],
    datas=[
        ('src/nuristat/resources', 'nuristat/resources'),
    ],
    ...
    excludes=[
        ...
        # 미사용 딥러닝/ML 스택 (로컬 환경에서 전이적으로 끌려오는 대용량 — nuristat 미사용)
